...
## Training
```bash
python feature_vector_generation/train.py
```
## Inference via scripts
```bash
python feature_vector_generation/predict.py
```

## Endpoints
```bash
uvicorn feature_vector_generation.app:app --host 0.0.0.0 --port 5000 --reload
...
1. Build image
```bash
docker build -t fvg:latest -f Dockerfile .
```
2. Run container
```bash
docker run -d -p 5000:5000 -p 6006:6006 --name fvg fvg:latest
```
...
```
docker run -p 5000:5000 --name fvg fvg:latest
```

• Get inside container if using `CMD ["/bin/bash"]`
```
docker run -p 5000:5000 -it fvg /bin/bash
```
