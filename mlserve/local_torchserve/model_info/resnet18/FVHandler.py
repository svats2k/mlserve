...
def compress_nparr(nparr: np.ndarray) -> Tuple[bytes, int, int]:
    ...
    logger.info(f"Input length: {len(uncompressed)}, compressed length: {len(compressed)}")
...
class FVHandlerSingle(FVHandler):
    ...
    def preprocess(self, data) -> np.ndarray:
        ...
        logger.info(f"Batch shape: {imgs_np.shape} and torch arr: {images.shape}")
        return images
    ...
    def postprocess(self, data: np.ndarray) -> List[str]:
        ...
        logger.info(f"Returning : {type(compressed_byte_string)} comprising of {data.shape}")
...
class FVHandlerBatch(FVHandler):
    ...
    def preprocess(self, requests) -> np.ndarray:
        logger.info(f"incoming data type: {type(requests)}, incoming length: {len(requests)}")
        images_batch = []
        for idx, data in enumerate(requests):
            input_data = data.get('data') or data.get('body')
            imgs_np = uncompress_nparr(input_data)
    ...
    def inference(self, image_batches: List[torch.Tensor], *args, **kwargs) -> np.ndarray:
        ...
        for image_batch in image_batches:
            logger.info(f"In inference image batch shape: {image_batch.shape}")
            h = self.extraction_layer.register_forward_hook(self.save_output_hook('fvec'))
            with torch.no_grad():
    ...
    def postprocess(self, data: List[np.ndarray]) -> List[str]:
        ...
        logger.info(f"Number of output batches: {len(data)} and type: {type(data)}")
