...
from videoken.logger import logger
from videoken.utils.misc import stopwatch
...
class Img2Vec():
    ...
    def get_vecs(self, imgs_batch:np.ndarray) -> np.ndarray:
        images:List[torch.Tensor] = [self.normalize(self.to_tensor(self.scaler(Image.fromarray(image)))) for image in imgs_batch]
        images = torch.stack(images).to(device=self.device, dtype=torch.float32)
